...
with open('ImageThresholding_Segmentation\monedas', 'wb') as f:
    f.write(r.content)

#Image load
img=io.imread(os.path.join('ImageThresholding_Segmentation\monedas'))
...
ax0[1].grid(True)
fig0.savefig('ImageThresholding_Segmentation\subplot1.png')
fig0.tight_layout()
fig0.show()
...
ax[0][0].set_title('Mask 1 1\n(Otsu)', wrap=True, fontsize=10)
ax[0][0].imshow(bina_otsu, cmap='gray')
ax[0][0].axis('off')
ax[0][1].set_title('Mask 2\n(percentil)', wrap=True, fontsize=10)
ax[0][1].imshow(bina_per, cmap='gray')
ax[0][1].axis('off')
ax[0][2].set_title('Mask 3\n(Arbitrary threshold)', wrap=True, fontsize=10)
ax[0][2].imshow(bina_175, cmap='gray')
ax[0][2].axis('off')
ax[0][3].set_title('Mask 4\n(Choosen threshold)', wrap=True, fontsize=10)
...
ax[1][2].axis('off')
ax[1][3].set_title('Segmentation\nChoosen threshold', wrap=True, fontsize=10)
ax[1][3].imshow(bina_esc*img, cmap='gray')
ax[1][3].axis('off')
fig.savefig('ImageThresholding_Segmentation\subplotLAB.png')
fig.tight_layout()
plt.show()
...
#Data load
lista=glob.glob(os.path.join('ImageThresholding_Segmentation','Heart_Data', 'Data', '*.nii.gz'))
arch=nibabel.load(os.path.join('ImageThresholding_Segmentation','Heart_Data', 'Data', '4.nii.gz'))
...
for i in range(vol1.shape[2]-1): #Se visualizan los cortes del paciente 3
    ...
    plt.title(f' Tomography visualization: Patient 3, slice: {i}')
    plt.axis('off')
    plt.draw()
    plt.pause(0.000001)
    plt.clf()
...
for i in range(vol2.shape[2]-1):#Se visualizan los cortes del paciente 12
    ...
    plt.title(f' Tomography visualization: Patient 12, slice: {i}')
    plt.axis('off')
    plt.draw()
    plt.pause(0.000001)
    plt.clf()
...
for i in range(vol3.shape[2]-1):#Se visualizan los cortes del paciente 14
    ...
    plt.title(f' Tomography visualization: Patient 14, slice: {i}')
    plt.axis('off')
    plt.draw()
    plt.pause(0.000001)
    plt.clf()